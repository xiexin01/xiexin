...
portal.define([
	"text!oss/opb/bsdk/modules/busiPackConfig/templates/dialog/BatchDisabledDialog.html",
	...
	return portal.CommonView.extend({
		...
		events: {
			"click #js-batch-disabled-btn": "batchDisabled",
			"click #btn-bp-repo-search-by-cond": "loadGridDatas",
			"click #js-batch-disabled-cancel-btn": "onCancel"
		},		
		...
		afterRender: function() {
			...
			this.disabledGrid = this.$("#batch-disabled-grid").jqGrid({
				 autowidth: true, 
				 height: 350,
...
//			this.loadGridDatas();
		...
		onRowSelectCallback : function(e, rowid, state, checked){
			var checkRows = this.disabledGrid.jqGrid('getCheckRows');
			if(checkRows.length>0){
				
				this.$('#js-batch-disabled-btn').attr('disabled',false);
			}else{
				this.$('#js-batch-disabled-btn').attr('disabled',true);
			}
		...
		loadGridDatas : function(){
			...
			bpMakeAction.qryBPInst(this.inParam,this.wrap(function(datas){
				var result = datas || [];
				this.disabledGrid.jqGrid('reloadData',result);
			}));
		},
		batchDisabled :function(){
			...
			bpMakeAction.forbiddenPackage(param,this.wrap(function(data){ 
				fish.success(this.resource.OPB_BSDK_BP_FORBIDDEN_SUCCESS);
				this.trigger('batchDisabledEvent');
			}));
		},
